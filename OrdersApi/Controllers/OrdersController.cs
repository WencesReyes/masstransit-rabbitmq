    ...
    public class OrdersController : ControllerBase
    {
        ...
        public async Task<ActionResult<Order>> PostOrder(OrderModel model)
        {
            ...
            await _publishEndpoint.Publish(new OrderCreated
            {
                ...
                TotalAmount = createdOrder.OrderItems.Sum(o => o.Quantity * o.Price)
            }, ctx => {
                //set custom headers
                ctx.Headers.Set("my-custom-header-key", "custom-header-value");
                //set expiration time
                ctx.TimeToLive = TimeSpan.FromSeconds(10);
            });

            //await _publishEndpoint.Publish(new TestEvent());
